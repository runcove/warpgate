...
| 98 | `hardened-run.sh` | The source tree could not be delivered into the capped container. Produced only when a caller passes `--source` (`run-check.sh` does, for every capped check: `--source "$PWD" --workdir /src`) — either the `docker cp` itself failed, or it exited 0 but a known file (`$WORKDIR/Cargo.toml`) was not found inside the container when checked directly afterwards, meaning the copy landed in the wrong place or copied the wrong thing. Without this check a capped container that never received the source would run its check command against an empty directory and fail with an ordinary-looking, in-range exit code — indistinguishable from the code under test actually being broken. |
