...
| 93 | `cache-env.sh`, `check-schema-compat.sh`, `configure-cache.sh`, `hardened-run.sh`, `preflight.sh`, `run-check.sh`, `version.sh` | A required argument or piece of configuration is missing or unrecognised (a bucket name, `S3_ENDPOINT`, `HARDENED_RUN_IMAGE`, python3/PyYAML/docker/dockerd for `preflight.sh`, a check name, a `version.sh` subcommand/operand). `check-schema-compat.sh` adds three: no readable `checks.yaml`, no `upstream_tag` inside it, and no `oasdiff-severity.txt` — each one leaves the run with no baseline or no thresholds, and guessing either would produce a comparison against something nobody chose. It also uses 93 for a test hook (`SCHEMA_COMPAT_GEN`, `SCHEMA_COMPAT_OASDIFF`) found set inside real CI, mirroring `run-check.sh`: a hook that stands in for the thing being measured is a configuration error, not a result. Never bash's own `${1:?}`/`${2:?}` — that form exits 1, indistinguishable from an ordinary failure and outside this band entirely. |
...
| 96 | `check-schema-compat.sh`, `version.sh` | `check-schema-compat.sh` could not establish WHICH release to compare against, or could not get far enough to compare: the two records of the base release disagree (`upstream_tag` in `checks.yaml` against the tag encoded in a `cove-patches-vX.Y.Z` branch name — it prints both and prefers neither, because a comparison against the wrong release is worse than no comparison and nothing in the script can tell which side is stale); the tag does not resolve in this checkout (a shallow clone, or tags never fetched); its committed schemas could not be extracted; the current schema could not be generated; or `oasdiff` itself exited inside this band, which must be read as the tool failing rather than as a verdict about the API. `version.sh`'s `--next` could not establish that its view of existing `<base>-cove.*` tags is complete before trusting an empty result to mean "no cove release yet for this base". Covers a shallow clone (`git rev-parse --is-shallow-repository` != `false`) and a full clone whose fetch simply never brought tags (no `--tags`, a checkout step that skipped them) — the latter is caught by comparing against `git ls-remote`, since local history alone can't tell "no releases yet" from "my tags never arrived" apart. Also covers the remote query itself failing or timing out: a refusal costs a rerun, a wrong guess here republishes an existing release. |
...
| 98 | `hardened-run.sh` | The source tree could not be delivered into the capped container. Produced only when a caller passes `--source` (`run-check.sh` does, for every capped check: `--source "$PWD" --workdir /src --verify-file Cargo.toml`) — the target directory could not be created, the `docker cp` itself failed, or the post-copy verification found nothing where it looked: a named `--verify-file` (relative to `--workdir`) not present, or — when no `--verify-file` was given — the workdir simply empty (a deliberately weaker check: `hardened-run.sh` names no project, so it cannot assume any particular filename; a caller that can name one gets the stronger check). Without this, a capped container that never received the source would run its check command against an empty directory and fail with an ordinary-looking, in-range exit code — indistinguishable from the code under test actually being broken. |
| 99 | `check-lockfile.sh`, `check-schema-compat.sh`, `run-all-checks.sh` | The check (or the check-runner itself) found nothing to examine, and that is neither a pass nor a fail — only "nothing was proven". Distinct from 97: the tool(s) ran fine, there was simply nothing to point them at. `check-lockfile.sh` exits 99 when `find` matches zero `package-lock.json` files — the concrete case is a check running before source has been delivered into the capped container (the gap Task 7B closes; until then, or if that delivery ever breaks again, this is what an empty checkout looks like, and it must not read as either a clean pass or a code problem). `check-schema-compat.sh` exits 99 when one side of a comparison is missing or empty — the tag carries no schema at that path, or this tree produced none — because a comparison with one operand is not a pass, however clean the other side looks. `run-all-checks.sh` exits 99 the same way, one level up, if it enumerates zero checks from `checks.yaml` — either because `list-checks.py` itself failed (its exit status is checked directly, not left to a process substitution to swallow) or, as defence in depth, because it printed nothing at all despite exiting 0. |
