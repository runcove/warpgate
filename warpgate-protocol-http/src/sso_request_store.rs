...
//! of concurrent flows coexist. Binding each entry to the `UserSessionId` that
...
/// OAuth `state` and bound to the initiating [`UserSessionId`]. See the module
...
impl<V> SsoRequestStore<V> {
    ...
    pub async fn insert(&self, state: String, value: V, session_id: UserSessionId) {
    ...
    pub async fn take(&self, state: &str, session_id: &UserSessionId) -> Option<V> {
...
mod tests {
    ...
    async fn unit_two_concurrent_flows_in_same_session_are_both_retrievable() {
        ...
        let sid = UserSessionId(Uuid::new_v4());
        let now = base_ts();
    ...
    async fn unit_state_is_bound_to_initiating_session() {
        ...
        let store: SsoRequestStore<u32> = SsoRequestStore::new();
        let now = base_ts();
        let initiator = UserSessionId(Uuid::new_v4());
        let other = UserSessionId(Uuid::new_v4());
